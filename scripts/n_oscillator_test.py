...
#%%
# --- Parameters ---
omegas = np.array([[1.0, 1.01], [1.05, 1.06], [1.1, 1.11]])
N = len(omegas)
q = 1.0
gamma_meas = 5e-2
n_periods = 300
...
# --- Build the coupled optimal feedback (x-mode only) ---
...
# Variance solver (no-control baseline)
covs = array.variance_solver()

# Expectation solver with LQR cold damping on x-mode
result = array.expectation_solver(cold_damping_fn=common_force)

times = result['times']
xc = result['xc']  # (N, 2, n_times)

# --- Plot x-mode positions on one figure ---
plt.figure(figsize=(10, 5))
for j in range(N):
    plt.plot(times, xc[j, 0], label=rf'$\omega_{{x,{j+1}}} = {omegas[j,0]:.2f}$', alpha=0.8)
plt.xlabel(r'Time [$1/\omega$]')
plt.ylabel(r'$\langle x \rangle_c$ (x-mode)')
plt.title('Conditional mean positions with common LQR feedback (x-mode)')
plt.legend()
plt.tight_layout()
plt.show()
...
#%%
N = 2
omegas = np.array([[1.0, 1.01], [1.05, 1.06]])
eta = 0.2
gamma_BA = 0.18
n_thermal = 10
...
n_traj = 10
# Initial conditions: (n_traj, N, 2, 3) — [oscillator, mode, xc/pc/photocurrent]
initial_conditions = np.zeros((n_traj, N, 2, 3))
initial_conditions[:, :, :, 0] = np.random.randn(n_traj, N, 2) * np.sqrt(initial_temperature + 0.5)
initial_conditions[:, :, :, 1] = np.random.randn(n_traj, N, 2) * np.sqrt(initial_temperature + 0.5)
...
for g_fb in feedback_gains:
    ...
    for i in range(n_traj):
        result = osc_array.expectation_solver(
            cold_damping_fn=common_force_fn,
            initial_conditions=initial_conditions[i],
            gamma_fb=g_fb,
        )
        # Per-mode n_bar: (N, 2, n_times), sum over modes for per-oscillator
        n_bar_mode = osc_array.find_n_bar(result['xc'], result['pc'], result['Vxx'], result['Vpp'])
        n_bar = n_bar_mode.sum(axis=1)  # (N, n_times)
...
plt.tight_layout()
plt.title(rf'Two oscillators with common feedback -- $\omega_1 = {omegas[0,0]}$, $\omega_2 = {omegas[1,0]}$')
...
if os.path.exists(weights_path):
    # Architecture: 4*N inputs (2D modes), 2 outputs (u_cd, u_param)
...
n_traj_nn = 10
feedback_gains_rl = [0.5, 1.0, 2.0, 5.0]
for g_fb in feedback_gains_rl:
    ...
    for i in range(n_traj_nn):
        result = osc_array.expectation_solver(
            cold_damping_fn=common_force_fn_nn,
            initial_conditions=initial_conditions[i],
            gamma_fb=g_fb,
        )
        n_bar_mode = osc_array.find_n_bar(result['xc'], result['pc'], result['Vxx'], result['Vpp'])
